...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	#creates an array like ([x],[y]) where all TODOMask = 0
	index = np.where(TODOMask == 0)
	for r in range(ssd_rows):
		for c in range(ssd_cols):

			#calculate the position for the tempTexture array
			Txt_x = r + index[0]
			Txt_y = c + index[1]

			#array where mask = 0
			tempHole = TODOPatch[index[0], index[1]]
			#array with
			tempTexture = textureIm[Txt_x, Txt_y]
			#calculating the SSD
			SSD[r,c] = np.sum((((1.0 * tempTexture - 1.0 * tempHole))**2))
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	for i in range(patchSize):
		for j in range(patchSize):
				#calculating new locations for the swap
				iHoleIm = i + iPatchCenter - patchL
				jHoleIm = j + jPatchCenter - patchL

				iTextureIm = i + iMatchCenter - patchL
				jTextureIm = j + jMatchCenter - patchL
				#replaving imHole with the Match Patch.
				imHole[iHoleIm][jHoleIm] = textureIm[iTextureIm][jTextureIm]
...
im = Image.open('bricks.jpg').convert('RGB')
